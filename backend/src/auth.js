import { tokenHash, verifyPassword, newOpaqueToken } from "./crypto.js";
...
export async function authenticate(db, { schoolSlug, login, password, remember = false, ip, userAgent }, appConfig) {
  ...
  const ttlHours = remember ? appConfig.rememberSessionTtlHours : appConfig.sessionTtlHours;
  const expiresAt = new Date(Date.now() + ttlHours * 3_600_000);
  const session = await client.query(
    `INSERT INTO sessions (user_id, school_id, token_hash, expires_at, ip_address, user_agent)
     VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`,
    [user.user_id, user.school_id, tokenHash(token), expiresAt, ip, userAgent]
  );
  return { token: `${school.id}.${session.rows[0].id}.${token}`, expiresAt, school: { id: user.school_id, slug: user.slug }, user: { id: user.user_id, roles: user.roles } };
  });
}
...
export async function getSession(db, value) {
  ...
  const result = await client.query(
    `SELECT se.id, se.user_id, se.school_id, s.slug, array_agg(m.role) AS roles
     ...
     WHERE se.id = $1 AND se.token_hash = $2 AND se.revoked_at IS NULL AND se.expires_at > now()
