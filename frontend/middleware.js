...
/**
 * Security middleware — runs on every request before rendering.
 *
 * Responsibilities:
 * 1. Generate a per-request CSP nonce (prevents XSS via inline scripts)
 * 2. Set Content-Security-Policy with the nonce
 * 3. Set HSTS and other headers not already in next.config.mjs
 *
 * Next.js Turbopack injects inline scripts — we use nonces (not unsafe-inline)
 * so that only our own scripts execute, not injected attacker scripts.
 */
export function middleware(request) {
  ...
  const csp = [
    ...
    // Report violations (add a report-uri endpoint later)
